...
PROMPT = """Read briefing_packet.json, cycle_orders.json, research_program.json, reflection_packet.json, and current_cycle_status.json. You own the strategy decisions for this cycle. The pipeline only executes what you choose and returns results. HARD CONSTRAINT: every spec you write must stay inside the supported/fetchable backtest universe from briefing_packet.json and cycle_orders.json. You are allowed to choose the best-fit asset/timeframe from the validation basket based on thesis; do not confine yourself to the primary lane if another basket lane fits better. Design strategies as market mechanism -> component roles -> testable rules -> failure modes -> refinement path, not decorative indicator stacks. For every strategy, include a mandatory pre-submit reasoning block covering: market behavior targeted, why edge should exist, asset/timeframe fit, indicator role map, expected regime, likely failure mode, first refinement path, expected trade density, expected outcome pattern, what would count as success, and what would count as failure. Each strategy may request a multi-lane test basket, not just one lane. For every chosen lane, explain why that asset/timeframe belongs in the test, what result would justify continuing, what result would justify rotating away, and what you expect to happen there if the thesis is actually right. Encode those lanes in the strategy spec using validation_basket or test_lanes so the backtester can run all requested lanes. Run the density sanity check before submission: plausible trade count, contradictory filters, event sparsity, and asset/timeframe mismatch. Treat indicators as components with roles, not standalone strategies. Write the FULL batch of strategy specs for this cycle to artifacts/strategy_specs/. Do not generate cosmetic variants like EMA 9 -> 10 unless you explicitly justify the expected effect. Do not write or update a per-cycle journal as part of the live research cycle. Hard contract: if mode is explore, produce 2-4 materially distinct specs across the best-fit lanes in the allowed basket; if mode is exploit, produce 1-3 focused variants of the named passing strategy. Do not drift into one lonely spec per cycle unless minimum_spec_count is 1 and the evidence justifies it. All spec files for the cycle must be written before you finish. Also write/update agents/quandalf/memory/current_cycle_status.json with the SAME cycle_id as cycle_orders.json for this run. Do not increment it. Write: cycle_id, mode, research_direction, minimum_spec_count, maximum_spec_count, spec_paths, specs_produced, exploration_targets, iterate_target, new_families, iterated_families, abandoned_families, next_cycle_focus, and rationale. Treat 0-trade grammar as a red flag and avoid reusing it. Your live working context is briefing packet + cycle orders + research program + reflection packet + current cycle status."""
